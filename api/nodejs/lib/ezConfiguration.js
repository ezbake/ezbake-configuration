...
EzConfiguration.prototype._decryptValues = function(encryptedKeys){
  if(_.isEmpty(encryptedKeys)) return;
  var envKey = this.properties[constants.EZBAKE_SHARED_SECRET_ENVIRONMENT_VARIABLE];
  if(_.isEmpty(envKey)) return;
  var sharedSecret = process.env[envKey];
  if(_.isEmpty(sharedSecret)) return;
