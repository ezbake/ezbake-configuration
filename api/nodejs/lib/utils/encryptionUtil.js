...
function EncryptionUtil(encryptionImplementation){
  ...
  if(!_.isNull(encryptionImplementation) && !_.isUndefined(encryptionImplementation) && encryptionImplementation.isEncryptionImplementation){
    this.encryptionImplementation = encryptionImplementation;
  }
}

/**
 * Tests whether passed in value is encrypted based on pre and postfixes set in constants file
...
EncryptionUtil.prototype.isEncrypted = function(value){
  var startsWith = new RegExp(this.constants.ENCRYPTED_VALUE_PREFIX_REGEX);
  var endsWith = new RegExp(this.constants.ENCRYPTED_VALUE_POSTFIX_REGEX);
...
EncryptionUtil.prototype._wrapEncryptedValue = function(value){
  return this.constants.ENCRYPTED_VALUE_PREFIX + value + this.constants.ENCRYPTED_VALUE_POSTFIX;
};

EncryptionUtil.prototype._unwrapEncryptedValue = function(value){
  return value.substring(this.constants.ENCRYPTED_VALUE_PREFIX.length, value.length - this.constants.ENCRYPTED_VALUE_POSTFIX.length);
};
